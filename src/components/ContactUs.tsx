...
function LeftPanel() {
  return (
    <div className="flex flex-col bg-white relative">
      ...
      <div className="relative mt-[-40px] md:mt-[-60px] ml-4 md:ml-6 z-10 flex items-end gap-0"></div>
...
function CenterPanel() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://dahlia-contact-api.onrender.com/api/contact', {
        method: 'POST',
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4">
            ...
            <button
              ...
              className="w-full bg-[#c9a070] hover:bg-[#b8905f] disabled:opacity-60 text-white font-semibold py-3 rounded-lg tracking-wider text-sm uppercase transition flex items-center justify-center gap-2"
            >
